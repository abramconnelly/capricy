...
* `activity` is the activity, identified by a unique `activity_id`
  - custom activities can be renamed (done)
  - custom activities are identified by a 'base' `activity_id` with a `uuid` autogenerated
* `icons` are the pictographic representation, identified by the unique `icon_id`
  - don't store image location in db as it's site specific
* many different activities can be named the same thing
* many different activities can have the same icon
* many different activities can have the same name and the same icon
* the config screen should have the ability to import new icons or remove icons
  - keep the 'add icon/activity' interface simple
* 'add activity' should be able to rename custom activities simply
  - 'greyed out' system activities (can't rename)

TODO:

* generate random data to see/test
* ability to add a new entry from timeline
* stats
